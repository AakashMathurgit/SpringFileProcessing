...
def main():
    ...
    mh = "'C:\\Users\\aakas\\OneDrive\\Desktop\\WEb development\\apache-maven-3.9.9'"
    ...
    is_engine_start_up_success = start_engines(engine_ports=engine_ports, mh=mh, jh=jh)
    start_time = time.time()
    is_driver_start_up_success = start_driver(engine_ports=engine_ports, mh=mh, jh=jh)
    end_time = time.time()
...
def start_engines(engine_ports: list, mh: str, jh: str):
    os.system(f"java -version")
    try:
        for engine_port in engine_ports:
            exit_code = os.system(f"start_engine.sh {engine_port} --mh {mh} --jh {jh}")  # command to run engine at the given port

        return True
    except Exception as e:
        print(f"Failed to start engines at ports: {engine_ports} --mh {mh} --jh {jh}")
        return False


def start_driver(engine_ports: list,  mh: str, jh: str):
    try:
        exit_code = os.system(
            f"start_driver.sh {engine_ports[0]} {engine_ports[1]} {engine_ports[2]} --mh {mh} --jh {jh}")  # command to run driver at the given port
        return True
    except Exception as e:
